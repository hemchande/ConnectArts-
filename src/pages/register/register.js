...
function RegisterPage() {
    ...
    const createAccountLink = async (accountId) => {
      try {
        ...
        const response = await axios.post(`http://localhost:4000/routes/create-account-link?accountId=${accountId}`)
        ...
        signUp(email, password)
          .then((userCredential) => {
            ...
            if (resume){
             ...
             axios.post('http://localhost:4000/routes/adduserwithResume', formData)
            .then(response => {
    ...
    <div className={useStyles.formContainer}>
      ...
      <Divider className={useStyles.divider} />
        ...
        <label>First Name : </label>   
            <input type="firstname" value={firstName}  placeholder="Enter First Name" name="firstname" onChange={(e) => setFirstName(e.target.value)}  required/> 
            <br></br>
            <div/>
        <label>Last Name : </label>   
            <input type="lastname" value={lastName}  placeholder="Enter Last Name" name="lastname" onChange={(e) => setLastName(e.target.value)} required/> 
            <br></br>
            <div/>

        <label>Email : </label>   
            <input type="text" value={email} placeholder="Enter Email" name="email" onChange={(e) => setEmail(e.target.value)} size = {5} required/> 
            <br></br>
            <div/>
    ...
    padding: '5px',
        ...
        {role == "Reviewer" || role == "Performer/Reviewer" ? (
          <div>
            ...
            <input type="number" id="desired-pay-rate" name="desired-pay-rate" value={desiredPayRate} onChange={handleDesiredPayRateChange} />
          </div>
        ) : null}
        {role === "Performer" || role === "Performer/Reviewer"? (
          <div>
            <label htmlFor="desired-pay-range">Desired Pay Range:</label>
            <input type="number" id="desired-pay-range-start" name="desired-pay-range-start" value={desiredPayRange[0]} onChange={handleDesiredPayRangeStartChange} />
            <input type="number" id="desired-pay-range-end" name="desired-pay-range-end" value={desiredPayRange[1]} onChange={handleDesiredPayRangeEndChange} />
          </div>
        ) : null}
...
<label>
        ...
        <label htmlFor="checkboxes" style={{fontSize: "1.1rem"}}>
          Reviewer preferences of the importance of each performance subcategory to you when reviewing a performance :</label>
...
<Grid container spacing={1}>
        ...
        <Grid item>
          ...
          <div className={`info-description ${showInfo === "technique" ? "show" : ""}`}>
      <p> How the body moves and how the space is used eg pathways, body positioning levels, directions, size of movements, patterns, spatial design, and shape of movements and poses; 
